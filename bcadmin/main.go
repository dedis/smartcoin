...
func create(c *cli.Context) error {
	...
	req, err := transaction.DefaultGenesisMsg(transaction.CurrentVersion, r,
	...
	cl, resp, err := transaction.NewLedger(req, false)
	if err != nil {
		return err
	}
...
func link(c *cli.Context) error {
	...
	var cl *transaction.Client
	var cc *transaction.ChainConfig
	for _, si := range roster.List {
		ids, err := fetchChains(si, byzcoinFetcher, skipchainFetcher)
		if err != nil {
		...
		if found {
			cl = transaction.NewClient(id, *onet.NewRoster([]*network.ServerIdentity{si}))
...
func byzcoinFetcher(si *network.ServerIdentity) ([]skipchain.SkipBlockID, error) {
	cl := transaction.NewClient(nil, onet.Roster{})
...
func latest(c *cli.Context) error {
	...
	p, err := cl.GetProofFromLatest(smartc.ConfigInstanceID.Slice())
	if err != nil {
		return err
	}
	...
	if c.Bool("header") {
		var header transaction.DataHeader
...
func getBcKey(c *cli.Context) (cfg lib.Config, cl *transaction.Client, signer *darc.Signer,
	proof transaction.Proof, chainCfg transaction.ChainConfig, err error) {
	...
	pr, err := cl.GetProofFromLatest(smartc.ConfigInstanceID.Slice())
	if err != nil {
...
func getBcKeyPub(c *cli.Context) (cfg lib.Config, cl *transaction.Client,
	signer *darc.Signer, proof transaction.Proof, chainCfg transaction.ChainConfig,
...
func updateConfig(cl *transaction.Client, signer *darc.Signer, chainConfig transaction.ChainConfig) error {
	...
	ctx, err := cl.CreateTransaction(transaction.Instruction{
		InstanceID: smartc.ConfigInstanceID,
		Invoke: &transaction.Invoke{
			ContractID: smartc.ContractConfigID,
			Command:    "update_config",
			Args:       transaction.Arguments{{Name: "config", Value: ccBuf}},
		},
...
func mint(c *cli.Context) error {
	...
	h.Write(pubBuf)
	account := transaction.NewInstanceID(h.Sum(nil))
	...
	if !p.Proof.InclusionProof.Match(account.Slice()) {
		...
		counters[0]++
		ctx, err := cl.CreateTransaction(transaction.Instruction{
			InstanceID: transaction.NewInstanceID(cfg.AdminDarc.GetBaseID()),
			Spawn: &transaction.Spawn{
				ContractID: smartc.ContractDarcID,
		...
		counters[0]++
		ctx, err = cl.CreateTransaction(transaction.Instruction{
			InstanceID: transaction.NewInstanceID(d.GetBaseID()),
			Spawn: &transaction.Spawn{
...
func darcCdesc(c *cli.Context) error {
	...
	ctx, err := cl.CreateTransaction(transaction.Instruction{
		InstanceID:    transaction.NewInstanceID(d2.GetBaseID()),
		Invoke:        &invoke,
...
func debugList(c *cli.Context) error {
	...
	for _, url := range urls {
		...
		resp, err := transaction.Debug(url, nil)
		if err != nil {
			log.Error(err)
			continue
		}
		sort.SliceStable(resp.Byzcoins, func(i, j int) bool {
			var iData transaction.DataHeader
			var jData transaction.DataHeader
		...
		for _, rb := range resp.Byzcoins {
			...
			headerGenesis := transaction.DataHeader{}
			headerLatest := transaction.DataHeader{}
...
func debugDump(c *cli.Context) error {
	...
	resp, err := transaction.Debug(c.Args().First(), &bcid)
	if err != nil {
		log.Error(err)
		return err
	}
...
func debugRemove(c *cli.Context) error {
	...
	bcid := skipchain.SkipBlockID(bcidBuf)
	err = transaction.DebugRemove(si, bcid)
	if err != nil {
		return err
	}
...
func darcAdd(c *cli.Context) error {
	...
	rules := darc.NewRules()
	rules.AddRule("invoke:"+smartc.ContractDarcID+".evolve", adminExpr)
	...
	if c.Bool("unrestricted") {
		err = rules.AddRule("invoke:"+smartc.ContractDarcID+".evolve_unrestricted", adminExpr)
		if err != nil {
			return err
		}
	}
	...
	instID := transaction.NewInstanceID(dSpawn.GetBaseID())
