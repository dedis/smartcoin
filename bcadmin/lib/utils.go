...
func GetDarcByString(cl *transaction.Client, id string) (*darc.Darc, error) {
...
func GetDarcByID(cl *transaction.Client, id []byte) (*darc.Darc, error) {
...
func ExportTransaction(tx transaction.ClientTransaction) error {
	...
	// pointers.
	instrs := make([]transaction.Instruction, len(tx.Instructions))
	for i, instr := range tx.Instructions {
		instrs[i] = transaction.Instruction{
...
func WaitPropagation(c *cli.Context, cl *transaction.Client) error {
