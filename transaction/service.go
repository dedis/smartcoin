// Package transaction implements the ByzCoin ledger.
package transaction
...
func init() {
	...
	err = smartcI.RegisterGlobalContract(smartcI.GetContractConfigID(), smartcI.ContractConfigFromBytes)
	if err != nil {
		panic(err)
	}
	err = smartcI.RegisterGlobalContract(smartcI.GetContractConfigID(), smartcI.ContractSecureDarcFromBytes)
	if err != nil {
		panic(err)
	}
	// err = smartc.RegisterGlobalContract(smartc.ContractDeferredID, smartc.ContractDeferredFromBytes)
	// if err != nil {
	// 	panic(err)
	// }
	// err = smartc.RegisterGlobalContract(smartc.ContractNamingID, smartc.ContractNamingFromBytes)
	// if err != nil {
	// 	panic(err)
	// }
}
...
type Service struct {
	...
	*onet.ServiceProcessor
	...
	contracts ismartc.ContractRegistry
...
func (s *Service) CreateGenesisBlock(req *CreateGenesisBlock) (
	*CreateGenesisBlockResponse, error) {
	...
	spawnGenesis := &Spawn{
		ContractID: smartcI.GetContractConfigID(),
		Args: Arguments{
	...
	ctx := ClientTransaction{
		Instructions: []Instruction{
			{
				InstanceID: NewInstanceID([]byte(smartcI.GetContractConfigID())),
				Spawn:      spawnGenesis,
			},
		},
	}
...
func (s *Service) LoadGenesisDarc(scID skipchain.SkipBlockID) (*darc.Darc, error) {
	...
	darc, err := getInstanceDarc(st, NewInstanceID([]byte(smartcI.GetContractConfigID())), config.DarcContractIDs)
...
// contractName.
func (s *Service) GetContractConstructor(contractName string) (func(in []byte) (ismartc.Contract, error), bool) {
...
// TODO
func (t transaction) GetPairingSuite() pairing.Suite {
	return nil
}

// TODO
func (t transaction) GetServiceName() string {
	return ""
}
...
func (s *Service) GetContractInstance(contractName string, in []byte) (ismartc.Contract, error) {
	fn, exists := s.contracts.Search(contractName)
	if !exists {
	...
	cwr, ok := c.(ismartc.ContractWithRegistry)
	if ok {
...
func (s *Service) executeInstruction(st ReadOnlyStateTrie, cin []Coin, instr Instruction, ctxHash []byte, scID skipchain.SkipBlockID) (scs StateChanges, cout []Coin, err error) {
	...
	if !exists {
		if smartcI.GetContractConfigID() == instr.InstanceID.String() {
			// Special case 1: first time call to
			// genesis-configuration must return correct contract
			// type.
			contractFactory, exists = s.GetContractConstructor(smartcI.GetContractConfigID())
	...
	if sc, ok := c.(ismartc.ContractWithRegistry); ok {
	...
	// case SpawnType:
	// 	changes, err = c.Spawn(gs, instr, trans)
	// case InvokeType:
	// 	changes, err = c.Invoke(gs, instr, trans)
	// case DeleteType:
	// 	changes, err = c.Delete(gs, instr, trans)
	// default:
	// 	return nil, nil, xerrors.New("unexpected contract type")
	// }

	// TODO: create the state change with the change
...
func newService(c *onet.Context) (onet.Service, error) {
	s := &Service{
		...
		contracts:              *smartcI.GetContractRegistry(),
