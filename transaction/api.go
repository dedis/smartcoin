...
// func (c *Client) GetDeferredData(instrID InstanceID) (*smartc.DeferredData, error) {
...
// func (c *Client) GetDeferredDataAfter(instrID InstanceID, barrier *skipchain.SkipBlock) (*smartc.DeferredData, error) {
...
// 	}
// 	var result smartc.DeferredData
...
func (c *Client) GetGenDarc() (*darc.Darc, error) {
	...
	if contract != smartcI.GetContractConfigID() {
...
func DefaultGenesisMsg(v Version, r *onet.Roster, rules []string, ids ...darc.Identity) (*CreateGenesisBlock, error) {
	...
	if err := rs.AddRule("invoke:"+"config"+"."+"update_config", ownerExpr); err != nil {
		log.Error(err)
		return nil, xerrors.Errorf("adding rule: %v", err)
	}
	if err := rs.AddRule("spawn:"+"darc", ownerExpr); err != nil {
		return nil, xerrors.Errorf("adding rule: %v", err)
	}
	if err := rs.AddRule("invoke:"+"darc"+"."+"evolve", ownerExpr); err != nil {
		return nil, xerrors.Errorf("adding rule: %v", err)
	}
	if err := rs.AddRule("invoke:"+"darc"+"."+"cmdDarcEvolveUnrestriction", ownerExpr); err != nil {
		return nil, xerrors.Errorf("adding rule: %v", err)
	}
	if err := rs.AddRule("_sign", ownerExpr); err != nil {
		return nil, xerrors.Errorf("adding rule: %v", err)
	}
	// if err := rs.AddRule("spawn:"+smartc.ContractNamingID, ownerExpr); err != nil {
	// 	return nil, xerrors.Errorf("adding rule: %v", err)
	// }
	...
	d.Rules.AddRule(darc.Action("invoke:"+smartcI.GetContractConfigID()+".view_change"), expression.InitOrExpr(rosterPubs...))
	...
	m := CreateGenesisBlock{
		...
		DarcContractIDs: []string{smartcI.GetContractConfigID()},
	}
	return &m, nil
}
