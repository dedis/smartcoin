...
func TestSecureDarc(t *testing.T) {
	...
	genesisMsg, err := transaction.DefaultGenesisMsg(transaction.CurrentVersion, roster, []string{}, signer.Identity())
	require.Nil(t, err)
	gDarc := &genesisMsg.GenesisDarc
	genesisMsg.BlockInterval = time.Second
	cl, _, err := transaction.NewLedger(genesisMsg, false)
	require.Nil(t, err)
	...
	ctx, err := cl.CreateTransaction(transaction.Instruction{
		InstanceID: transaction.NewInstanceID(gDarc.GetBaseID()),
	...
	log.Info("evolve to add rules - fail")
	{
		...
		ctx2, err := cl.CreateTransaction(transaction.Instruction{
			InstanceID: transaction.NewInstanceID(secDarc.GetBaseID()),
	...
	log.Info("evolve_unrestricted fails with the wrong signer")
	{
		...
		ctx2, err := cl.CreateTransaction(transaction.Instruction{
			InstanceID: transaction.NewInstanceID(myDarc.GetBaseID()),
			Invoke: &transaction.Invoke{
				ContractID: ContractDarcID,
